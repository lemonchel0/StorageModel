...
void Database::Database_to_file(const std::string &file_items, const std::string &file_recipe){
	...
	auto t=recipe.root;
	recipe.printt(recipe.root,ofr);
	ofi.close();
	ofr.close();
}

void Database::Add_detail(const std::string &item_name,int kolv , LinkedList recip){
	avalableitems[item_name]=kolv;
	recipe.insert(item_name, recip);
}

void Database::delete_detail(const std::string &item_name, int kolv){
	avalableitems[item_name]-=kolv;
}
